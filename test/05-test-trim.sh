...
assertCmdEq "$CSV -ij --trim=none < $SAMPLE/trim-noeol.csv" "$expectedOutput" \
...
assertCmdEq "$CSV -ij < $SAMPLE/trim-noeol.csv" "$expectedOutput" \
...
assertCmdEq "$CSV -ij --trim=lines < $SAMPLE/trim-noeol.csv" "$expectedOutput_trim" \
...
assertCmdEq "$CSV --trim -ij < $SAMPLE/trim-noeol.csv" "$expectedOutput_trim" \
...
assertCmdEq "$CSV -ij --trim=lines < $SAMPLE/trim-q-noeol.csv" "$expectedOutput_trim_q" \
...
define expectedOutput_ws <<-EOT
	...
	,[""]]
EOT
assertCmdEq "$CSV -ij --trim=lines < $SAMPLE/whitespace.csv" "$expectedOutput_ws" \
